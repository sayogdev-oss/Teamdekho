...
class HtmlInjector {
    ...
    getInjectData() {
        return {
            ...
            OG_SITE_NAME: this.config?.og?.siteName || 'TeamDekho',
            OG_TITLE: this.config?.og?.title || 'Click the link to make a call.',
            OG_DESCRIPTION:
                this.config?.og?.description ||
                'TeamDekho calling provides real-time video calls, messaging and screen sharing.',
            OG_IMAGE: this.config?.og?.image || 'https://sfu.teamdekho.com/images/teamdekhosfu.png',
            OG_URL: this.config?.og?.url || 'https://sfu.teamdekho.com',
