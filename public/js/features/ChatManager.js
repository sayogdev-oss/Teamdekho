...
class ChatManager {
    ...
    sendChatReaction(msgListId, emoji) {
        ...
        const action = peers.includes(rc.peer_name) ? 'remove' : 'add';
        this.applyReactionToElement(msgEl, emoji, rc.peer_name, action);
        if (msgId) {
            rc.socket.emit('chatReaction', {
                msg_id: msgId,
                emoji: emoji,
                peer_name: rc.peer_name,
                peer_id: rc.peer_id,
    ...
    applyReactionToElement(msgEl, emoji, peerName, action = 'add') {
        ...
        if (action === 'add') {
            if (existing) {
                ...
                if (peerName === rc.peer_name) existing.classList.add('my-reaction');
            } else {
                const badge = document.createElement('span');
                badge.className = 'reaction-badge';
                if (peerName === rc.peer_name) badge.classList.add('my-reaction');
            ...
            if (existing) {
                ...
                if (peers.length === 0) {
                    ...
                    existing.querySelector('.reaction-count').textContent = peers.length;
                    existing.setAttribute('data-tooltip', peers.join(', '));
                    if (peerName === rc.peer_name) existing.classList.remove('my-reaction');
                }
            }
        }
    }
